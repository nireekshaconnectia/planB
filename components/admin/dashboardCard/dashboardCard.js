...
export default function DashboardCard() {
    ...
    const fetchDashboardData = useCallback(async () => {
        ...
        try {
            ...
            // Debug: Log orders data to understand structure
            console.log("📊 Dashboard Debug - Orders:", {
                total: orders.length,
                sample: orders[0],
                orderStatuses: [...new Set(orders.map(o => o.orderStatus || o.status))],
                paymentStatuses: [...new Set(orders.map(o => o.paymentStatus || o.paymentDetails?.status || o.payment?.status))],
                totalRevenueSample: orders.slice(0, 5).map(o => ({
                    orderId: o.orderId,
                    orderTotal: o.orderTotal,
                    orderStatus: o.orderStatus || o.status,
                    paymentStatus: o.paymentStatus || o.paymentDetails?.status || o.payment?.status
                })),
                revenueBreakdown: {
                    totalOrders: orders.length,
                    ordersWithTotal: orders.filter(o => o.orderTotal).length,
                    sumOfAllTotals: orders.reduce((sum, o) => sum + (parseFloat(o.orderTotal) || 0), 0),
                    sampleTotals: orders.slice(0, 10).map(o => parseFloat(o.orderTotal) || 0)
                }
            });
